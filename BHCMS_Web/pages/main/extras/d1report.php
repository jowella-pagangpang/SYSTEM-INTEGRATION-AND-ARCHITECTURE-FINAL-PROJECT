...
$objPHPExcel = $objReader->load("Classes/mytemplates-deworming1-report.xlsx");
...
FROM deworming1 WHERE 1stdose IS NULL OR 2nddose IS NULL");

while ($data = mysqli_fetch_array($deworming1st)) {
  $deworming1_id = strip_tags($data['deworming1_id']);
...
header('Content-Disposition: attachment;filename="Deworming_1-4_Patient_Report.xlsx"');
