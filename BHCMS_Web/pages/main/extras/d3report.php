...
$objPHPExcel = $objReader->load("Classes/mytemplates-deworming3-report.xlsx");
...
FROM deworming3 WHERE 1stdose IS NULL OR 2nddose IS NULL");

while ($data = mysqli_fetch_array($deworming3rd)) {
  $deworming3_id = strip_tags($data['deworming3_id']);
...
header('Content-Disposition: attachment;filename="Deworming_10-19_Patient_Report.xlsx"');
