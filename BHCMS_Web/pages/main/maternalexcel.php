...
$maternal = mysqli_query($con, "SELECT maternal_id, patientid, DATE_FORMAT(reg_date,'%m-%d-%Y') AS regdate, 
  ...
  FROM maternal INNER JOIN client ON maternal.patientid = client.id WHERE reg_date between '$startdate' AND '$enddate'");
...
header('Content-Disposition: attachment;filename="TCL-Maternal-Care-Report.xlsx"');
