...
$postpartum = mysqli_query($con, "SELECT postpartum_id, patientid, DATE_FORMAT(delivery_date,'%m-%d-%Y') AS deliverydate, 
  delivery_time, fname, minitial, lname, purok, address, 
  ...
  FROM postpartum INNER JOIN client ON postpartum.patientid = client.id WHERE delivery_date between '$startdate' AND '$enddate'");
...
while ($data = mysqli_fetch_array($postpartum)) {
  ...
  $delivery_time = strip_tags($data['delivery_time']);
  ...
  $cells++;
    ...
    $objPHPExcel->getActiveSheet()
        ...
        ->mergeCells('A' . ($cells + 2) . ':K' . ($cells + 2));
...
header('Content-Disposition: attachment;filename="TCL-Postpartum-Care-Report.xlsx"');
